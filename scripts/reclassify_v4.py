...
SYSTEM_PRIORITY = ["cardiovascular", "muscular", "skeletal", "nervous", "digestive", "respiratory", "urinary", "reproductive", "endocrine", "lymphatic", "integumentary"]
...
GENERIC_NAMES = [
    ...
    "muscle organ", "muscle of neck", "muscle of head", "nerve of", "bone of", "vessel of"
]
...
def classify(en, kanji):
    ...
    if any(x in en for x in ["nerve", "ganglion", "plexus", "brain", "cerebr", "thalamus", "hypothalamus", "pons", "medulla", "spinal cord", "hippocampus", "amygdala", "callosum", "matter", "hemisphere", "retina", "optic", "iris", "cornea", "lens", "ciliary", "sclera", "vitreous", "eyeball"]):
        return "nervous"
    if any(x in kj for x in ["神経", "脳", "丘脳", "終板", "乳頭体", "桥", "網膜", "角膜", "虹彩", "水晶体", "強膜", "脈絡膜"]):
        return "nervous"
        
    # Priority 3: Muscular
    if any(x in en for x in ["muscle", "musculature", "tendon", "fascia", "aponeurosis", "rectus", "oblique", "vastus", "trapezius", "pectoralis", "levator", "adductor", "flexor", "pronator", "iliococcygeus", "pubococcygeus", "puborectalis", "longus colli", "muscular", "sternothyroid", "cricothyroid", "thyrohyoid", "geniohyoid", "sternohyoid", "digastric", "stylohyoid"]):
        return "muscular"
    if any(x in kj for x in ["筋", "腱", "腱輪"]):
        return "muscular"

    # Priority 4: Skeletal
    if any(x in en for x in ["bone", "vertebra", "skull", "rib", "sternum", "xiphoid process", "spine", "sacrum", "coccyx", "phalanx", "clavicle", "scapula", "humerus", "radius", "ulna", "carpal", "metacarpal", "femur", "tibia", "fibula", "tarsal", "metatarsal", "patella", "hip bone", "hyoid", "skeletal", "condyle", "process"]):
        return "skeletal"
    if any(x in kj for x in ["骨", "椎", "肋", "剣状突起", "頭蓋", "突起"]):
        return "skeletal"
...
def translate(kanji, en):
    ...
    if kj.lower() == en.lower() and any(ord(c) < 128 for c in kj): return en.capitalize()
    ...
    res = res.replace("臍", "脐").replace("繋", "系").replace("鞁", "鞁") # skip obscure
...
with open('data/isa_element_parts.txt', 'r') as f:
    ...
    for row in reader:
        ...
        if fj not in fj_fma_map:
            fj_fma_map[fj] = []
        fj_fma_map[fj].append(fma)

new_data = {k: [] for k in SYSTEM_NAMES.keys()}
for fj_id in sorted(fj_fma_map.keys()):
    ...
    for fma in fmas:
        ...
        sys = classify(info['en'], info['kanji'])
        if sys != "other":
    ...
    # Best name: specific, has Kanji, longest name
    best_fma = fmas[0]
    best_score = -1
    for fma in fmas:
        info = fma_info.get(fma)
        ...
        score = 0
        if not any(g in name for g in GENERIC_NAMES):
            score += 100
        if any(ord(c) > 127 for c in kanji): # has Kanji/Japanese
            score += 50
        score += len(name)
    ...
    item = {
        "id": fj_id,
        "name": f"{nameCn} | {en}",
        "system": SYSTEM_NAMES[final_sys],
        "nameEn": en,
        "nameCn": nameCn
    }
    new_data[final_sys].append(item)
