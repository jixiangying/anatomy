...
def classify(en, kanji):
    ...
    if any(x in en for x in ["testis", "epididymis", "deferent duct", "seminal vesicle", "prostate", "penis", "scrotum", "ovary", "uterus", "uterine", "vagina", "vulva", "reproductive"]):
        return "reproductive"
    if any(x in kj for x in ["精巣", "副精巣", "輸精管", "精嚢", "前立腺", "陰茎", "卵巣", "子宫", "腟"]):
        return "reproductive"
...
for fj_id in sorted(fj_fma_map.keys()):
    ...
    # Find best system across all FMAs
    final_sys = "other"
    matched_systems = set()
    for fma in fmas:
    ...
    # Find best name (most specific FMA)
    best_fma = fmas[0]
    best_len = -1
    for fma in fmas:
        info = fma_info.get(fma)
        ...
        if not is_generic and len(name) > best_len:
            best_len = len(name)
            best_fma = fma
        elif best_len == -1 and len(name) > best_len:
            best_len = len(name)
            best_fma = fma
